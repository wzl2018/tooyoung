 		...
 		constructor:zturn,
 		 init:function(){
				...
				this.zturn.children(".zturn-item").each(function(index,element){	 		
							...
							var rt=1
							if((index-_self.X)>_self.num_li/2||(index-_self.X)<0&&(index-_self.X)>(-_self.num_li/2)){rt=-1}
							var i=Math.abs(index-_self.X);
							if(i>_self.num_li/2){i=parseInt(_self.X)+parseInt(_self.num_li)-index;}
...
var zturn=function(turn){
						...
						zturn.children(".zturn-item").each(function(index,element){
							...
							var rt=1
							if((index-X)>num_li/2||(index-X)<0&&(index-X)>(-num_li/2)){rt=-1}
							var i=Math.abs(index-X);
							if(i>num_li/2){i=X+num_li-index}
...
//						console.log(turn,X)
						...
						var rt=1
							if((index-X)>num_li/2||(index-X)<0&&(index-X)>(-num_li/2)){rt=-1}/
							var i=Math.abs(index-X);
							if(i>num_li/2){i=parseInt(X)+num_li-index;}
							if((index-X)<(-num_li/2)){i=num_li+index-X}
